        ...
        "plugins": [
            {
                ...
                "releases": [
                    {
                        "version": "0.2.0",
                        "host_version_spec": ">=0.2.0,<0.3.0",
                        "wheel_url": "https://example.invalid/host-camera-0.2.0.whl",
                    }
                ],
            },
            {
                ...
                "releases": [
                    {
                        "version": "0.2.0",
                        "host_version_spec": ">=0.2.0,<0.3.0",
                        "wheel_url": "https://example.invalid/host-microphone-0.2.0.whl",
                    }
                ],
            },
        ],
...
def test_status_reports_official_and_third_party_plugins(monkeypatch, capsys) -> None:
    monkeypatch.setattr(cli, "_host_version", lambda: "0.2.0")
    monkeypatch.setattr(cli, "_load_manifest", lambda: cli._parse_manifest(_manifest_payload()))
    monkeypatch.setattr(
        cli,
        "entry_points",
...
def test_manifest_uses_cache_when_network_fetch_fails(monkeypatch, tmp_path) -> None:
    ...
    cache_path.write_text(
        '{"schema_version": 1, "plugins": [{"plugin_id": "host-camera", "distribution": "modlink-plugin-host-camera", "display_name": "Host Camera", "summary": "Camera", "releases": [{"version": "0.2.0", "host_version_spec": ">=0.2.0,<0.3.0", "wheel_url": "https://example.invalid/host-camera.whl"}]}]}',
        encoding="utf-8",
    )
