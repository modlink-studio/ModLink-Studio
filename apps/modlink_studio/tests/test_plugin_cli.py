...
def test_status_reports_install_state(monkeypatch, capsys) -> None:
    installed_versions = {
        "modlink-plugin-host-camera": "0.2.0",
    ...
    monkeypatch.setattr(plugin_cli, "__version__", "0.2.0")

    exit_code = plugin_cli.main(["status"])

    assert exit_code == 0
    output = capsys.readouterr().out
    assert "modlink-studio 0.2.0" in output
    assert "host-camera" in output
    assert "installed=0.2.0" in output
    assert "host-microphone" in output
...
def test_install_downloads_release_asset_and_runs_pip(monkeypatch, tmp_path, capsys) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _tag: {
            "assets": [
                {
                    "name": "modlink_plugin_host_camera-0.2.0-py3-none-any.whl",
    ...
    monkeypatch.setattr(
        plugin_cli,
        "_download_asset",
        lambda _asset, _target_dir: tmp_path / "modlink_plugin_host_camera-0.2.0-py3-none-any.whl",
    )
    ...
    assert pip_calls == [
        (
            "install",
            str(tmp_path / "modlink_plugin_host_camera-0.2.0-py3-none-any.whl"),
        )
    ]
...
def test_uninstall_runs_pip_for_installed_plugin(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        plugin_cli,
        "_installed_version",
        lambda distribution_name: "0.2.0"
    ...
    assert "Uninstalled Host Microphone 0.2.0." in capsys.readouterr().out
