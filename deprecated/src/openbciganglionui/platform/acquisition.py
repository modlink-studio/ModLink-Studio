...
from .models import FrameEnvelope, PlatformErrorEvent
...
class AcquisitionTask(QObject):
    """Own recording lifecycle, stream capture, markers, segments, and writing."""
    ...
    def start_recording(self, request: RecordingRequest) -> None:
        ...
        selected_streams = request.stream_ids or tuple(
            descriptor.stream_id for descriptor in self._bus.descriptors()
        )
        ...
        self._subscription_tokens = [
            self._bus.subscribe(stream_id, self._on_frame)
            for stream_id in self._request.stream_ids
        ]
