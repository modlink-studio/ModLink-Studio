...
所以对 UI 来说，Core 更像“稳定的流提供者”，而不是必须深入理解的内部系统。

## 当前最值得依赖的稳定字段

如果页面希望尽量通用，最值得优先依赖的是这些字段：

- `descriptor.stream_id`
- `descriptor.payload_type`
- `descriptor.channel_names`
- `descriptor.metadata.get("unit")`
- `frame.data`

其中：

- `channel_names` 仍然是 UI 最适合直接依赖的共享约定
- `unit` 现在放在 `metadata` 里，因为它只对部分 payload 类型有意义
