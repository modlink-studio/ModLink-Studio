...
hero:
  ...
  tagline: 了解 0.3.1 的 PyPI 发布口径、driver 契约、回放能力和当前 widgets 宿主路线
  actions:
    - theme: brand
      text: 开始安装
      link: /install
...
features:
  - icon: 📦
    title: PyPI 安装
    details: 0.3.1 作为稳定性修订走 PyPI；TestPyPI rehearsal 已完成。
  - icon: 🔌
    title: Driver 接入
  ...
  - icon: ⚠️
    title: 0.3.1 发布
    details: 0.3.1 修复长时间录制自动停止 bug，加入 splash screen 和录制删除入口。
---
...
当前文档以 `0.3.1` 主线为准。这里描述的是 `0.3.1` 的当前边界：
...
`0.3.1` 的公开安装入口以 **PyPI** 为准；`TestPyPI rehearsal` 已完成，但 `TestPyPI` 不作为日常安装源。
