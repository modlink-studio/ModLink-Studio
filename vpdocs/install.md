...
本页汇总 `0.3.0rc2` 的安装目标、官方驱动安装方式和源码运行方式。

## 0.3.0rc2 发布状态

`0.3.0rc2` 当前处于预发布收口阶段：

- `0.3.0rc2` 是 `0.3.0` 的第二个 release candidate
- 正式公开发布渠道为 **PyPI**
...
`0.3.0rc2` 继续沿用 `0.2.0` 建立的纯 Python runtime 基线：
...
当前 UI 仍在适配期，但 backend 已经完成去 Qt 化；`0.3.0rc2` 重点增加 recording replay、analysis export 和外部插件 author skill，并修复 rc1 验证中发现的下拉菜单透明外框问题。

## 预发布安装命令

`0.3.0rc2` 是预发布版本，安装时建议明确指定版本：

```bash
python -m pip install --pre modlink-studio==0.3.0rc2
```
...
0.3.0rc2 不再提供独立 npm driver 脚手架。外部插件项目建议使用 `tools/modlink-plugin-author/SKILL.md` 作为 Claude Code / Codex 的可分发 skill，在插件自己的仓库里生成和维护 driver 代码。
...
`0.3.0rc2` 的 **TestPyPI rehearsal** 已完成。这一步只用于验证发布链路和安装命令，不作为普通用户安装入口。
