...
如果你不是在写 runtime，本页最重要的一句话是：

- UI 和应用层最好依赖 Core 暴露出来的稳定流模型，而不是直接依赖设备协议
...
`DriverRuntime` 负责把 driver 放到独立 `QThread` 上运行，并通过 invoker 把方法调用投递到这条线程。
...
1. driver 发出 `sig_frame(FrameEnvelope)`
2. `DriverRuntime` 接到这个 signal
3. `DriverPortal` 把它继续往上转
4. `StreamBus` 接收并分发
5. 录制后端和 UI 消费者分别订阅

对展示开发者来说，最重要的是最后两步，而不是设备底层怎么来的。
...
如果你在写 driver，其实不需要把 Core 内部全都看懂。

你最需要知道的是：

- 你的 `sig_frame` 会进入 `StreamBus`
- 你的 `StreamDescriptor` 会先被注册
- 你的生命周期方法在 worker thread 上同步执行

只要守住这些边界，driver 作者通常不需要继续往 Core 里钻。

## 对展示开发者真正有用的那部分

如果你在做页面、图表或面板，更建议把注意力放在这些约定上：
...
也就是说，UI 应该消费“已经稳定化的流模型”，而不是消费设备自己的底层概念。
