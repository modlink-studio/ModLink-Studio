...
def create_session(
    ...
    *,
    ...
    manifest_path = root_dir / "sessions" / resolved_session_id / "session.json"
...
def read_session(root_dir: Path, session_id: str) -> dict[str, Any]:
    return read_json(Path(root_dir) / "sessions" / session_id / "session.json")


def list_sessions(root_dir: Path) -> list[dict[str, Any]]:
    manifests: list[dict[str, Any]] = []
    base_dir = Path(root_dir) / "sessions"
...
def add_recording_to_session(
    ...
    *,
    ...
    root_dir = Path(root_dir)
    payload = read_session(root_dir, session_id)
    ...
    write_json(root_dir / "sessions" / session_id / "session.json", payload)
