...
DEMO_SETTINGS_SPEC = SettingsSpec(
    ...
    schema=group(
        ...
        label=str_setting(default="experiment-01"),
        storage=group(
...
def demo_nested_binding() -> SettingsService:
    # Example of a more structured schema
    spec = SettingsSpec(
        namespace="preview",
        schema=group(
            mode=enum_setting(values=("off", "simple", "advanced"), default="simple"),
            quality=group(
