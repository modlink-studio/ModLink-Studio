...
from ..adapters.field import FieldViewSettingsAdapter
from ..models import FieldPreviewSettings, normalize_preview_settings
...
class FieldSettingsController(QObject):
    ...
    def _load_and_normalize(self) -> FieldPreviewSettings:
        loaded = self._store.load(self._descriptor)
        if not isinstance(loaded, FieldPreviewSettings):
            loaded = FieldPreviewSettings()
        return self._normalize(loaded)

    def _normalize(self, settings: FieldPreviewSettings) -> FieldPreviewSettings:
        normalized = normalize_preview_settings(
            "field",
            settings,
            float(self._descriptor.nominal_sample_rate_hz or 1.0),
        )
        assert isinstance(normalized, FieldPreviewSettings)
        return normalized

    def _set_section_state(self, settings: FieldPreviewSettings) -> None:
        self._updating = True
        try:
