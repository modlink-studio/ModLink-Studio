...
当前仓库主线是 `0.3.0rc2`，对应 `0.3.0` 的当前 WIP / RC 阶段：

- `modlink_sdk` / `modlink_core` 已切成纯 Python runtime
- `0.2.0` 不兼容 `0.1.x` 的 Qt-style driver API
- UI 仍处于适配期，但 backend 已经从 Qt 运行时语义中拆开
- `0.3.0rc2` 已接入 recording replay、analysis export 和外部插件 author skill，并修复 rc1 验证中发现的下拉菜单透明外框问题
