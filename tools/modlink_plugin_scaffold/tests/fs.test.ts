...
describe("file generation", () => {
  test("writes the generated project tree", async () => {
    ...
    await expect(fs.readFile(path.join(generated.projectDir, "LICENSE"), "utf8")).resolves.toContain("MIT License");
    await expect(fs.readFile(path.join(generated.projectDir, "tests", "test_smoke.py"), "utf8")).resolves.toContain("create_driver");
  });
  ...
  test("throws when the target already exists unless overwrite is enabled", async () => {
    ...
    await expect(writeProjectFiles(spec!, cwd, "en", false)).rejects.toBeInstanceOf(ScaffoldExistsError);
    await expect(writeProjectFiles(spec!, cwd, "en", true)).resolves.toMatchObject({projectDir: existing});
  });
});
