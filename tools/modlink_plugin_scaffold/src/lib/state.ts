...
export function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "row.delta":
      return {
        ...state,
        rowIndex: Math.min(Math.max(state.rowIndex + action.delta, 0), Math.max(action.rowCount - 1, 0)),
      };
    case "row.clamp":
...
function replaceSelectedStream(draft: Draft, stream: StreamDraft): Draft {
  return {
    ...draft,
    streams: draft.streams.map((item, index) => (index === draft.selectedStreamIndex ? stream : item)),
  };
}

export function updateDraftField(draft: Draft, field: "pluginName" | "displayName" | "deviceId" | "providersText" | "dependenciesText", value: string): Draft {
  if (field === "pluginName") {
...
export function cycleDataArrival(draft: Draft, delta: number): Draft {
  ...
  return applyDataArrival(draft, dataArrivalOrder[(index + delta + dataArrivalOrder.length) % dataArrivalOrder.length] as DataArrival);
}

export function cycleDriverKind(draft: Draft, delta: number): Draft {
  const index = driverKindOrder.indexOf(draft.driverKind);
  return {...draft, driverKind: driverKindOrder[(index + delta + driverKindOrder.length) % driverKindOrder.length]};
}
...
export function cyclePayloadType(draft: Draft, delta: number): Draft {
  ...
  return replaceSelectedStream(draft, applyPayloadDefaults(stream, payloads[(index + delta + payloads.length) % payloads.length]));
}
...
export function cycleSelectedStream(draft: Draft, delta: number): Draft {
  ...
  return {
    ...draft,
    selectedStreamIndex: (draft.selectedStreamIndex + delta + draft.streams.length) % draft.streams.length,
  };
}
...
export function moveStream(draft: Draft, delta: -1 | 1): Draft {
  ...
  const streams = [...draft.streams];
  [streams[draft.selectedStreamIndex], streams[targetIndex]] = [streams[targetIndex], streams[draft.selectedStreamIndex]];
  return {...draft, streams, selectedStreamIndex: targetIndex};
}

export function updateSelectedStreamField(draft: Draft, field: keyof StreamDraft, value: string): Draft {
  const stream = draft.streams[draft.selectedStreamIndex];
  const updated = field === "payloadType" ? applyPayloadDefaults(stream, value as PayloadType) : {...stream, [field]: value};
...
export function getFirstErrorMessage(language: Parameters<typeof validateDraft>[0], draft: Draft): string | null {
